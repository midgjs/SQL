...
/*
문제5.
업무(job_id)별로 평균임금, 최고임금, 최저임금을 업무아이디(job_id)와 함께 출력하고 정렬순서
는 최저임금 내림차순, 평균임금(소수점 반올림), 오름차순 순입니다.
...
select  first_name,
        ...
        case when hire_date <= '02/12/31' then '창립멤버'
             when '03/01/01' <= hire_date and hire_date <= '03/12/31' then '03년입사'
             when '04/01/01' <= hire_date and hire_date <= '04/12/31' then '04년입사'
             when '05/01/01' <= hire_date then '상장이후입사' -- // else '상장이후입사' 
        end "optDate"
from employees
